...
class _DashboardScreenState extends State<DashboardScreen>
      ...
      if (isLoggedIn && (forceRefresh || !_initialDiariesFetchAttempted)) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          if (mounted && _isUserAuthenticated) {
            // 콜백 실행 시점에도 mounted 및 인증 상태 재확인
            _tryFetchInitialDiaries(
  ...
  Future<void> _tryFetchInitialDiaries({
    ...
    await _fetchDiariesAndUpdateState(
      ...
      isInitialLoad: actualInitialLoad,
    );
  }
  ...
  Future<void> _fetchDiariesAndUpdateState(
    ...
    try {
      ...
      final List<Diary> fetchedDiaries = await DiaryApi.instance
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          if (isInitialLoad)
            _initialDiariesFetchAttempted = true; // 초기 로드 시도 완료 플래그 설정
        });
      }
    } catch (e, stackTrace) {
      // 오류 처리
      print(
        'Error in _fetchDiariesAndUpdateState: ${e.runtimeType} - $e',
      ); // 콘솔에 오류 타입과 메시지 출력
        ...
        if (todayDiary != null) {
          ...
          _todayDiarySummary =
              todayDiary.summary.isNotEmpty
                  ? todayDiary.summary
                  : (todayDiary.content.length > 30
                      ? '${todayDiary.content.substring(0, 30)}...'
                      : todayDiary.content);
        } else {
  ...
  void _updateWeeklyFlameCount() {
    ...
    for (DateTime diaryDateUtc in _daysWithDiaryFromApi) {
      ...
      DateTime diaryDateOnly = DateTime.utc(
        diaryDateUtc.year,
        diaryDateUtc.month,
        diaryDateUtc.day,
      );
      DateTime startOfWeekOnly = DateTime.utc(
        startOfWeek.year,
        startOfWeek.month,
        startOfWeek.day,
      );
      DateTime endOfWeekOnly = DateTime.utc(
        endOfWeek.year,
        endOfWeek.month,
        endOfWeek.day,
      );
      ...
      if (!diaryDateOnly.isBefore(startOfWeekOnly) &&
          !diaryDateOnly.isAfter(endOfWeekOnly)) {
        count++;
      }
    }
    if (mounted) {
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                if (_isUserAuthenticated) ...[
                  // 로그인 상태일 때만 달력 및 요약 표시
                  if (_isLoadingDiaries &&
                      _cachedDiaries.isEmpty &&
                      !_initialDiariesFetchAttempted) // 다이어리 로딩 중
                    const Center(
                      child: Padding(
                    ...
                    Container(
                      key:
                          _calendarWidgetKey, // 캘린더 위젯에 GlobalKey 할당 (오버레이 위치 계산용)
                      child: CalendarWidget(
                        focusedDayForCalendar:
                            _focusedDayForCalendar, // 현재 포커스된 날짜
                        today: _today, // 오늘 날짜
  ...
  Widget _buildGreetingCard() {
    ...
    } else if (_initialDiariesFetchAttempted && _weeklyFlameCount == 0) {
      // 로그인 & 이메일 인증 & 이번 주 일기 없음
  ...
  void _handleCalendarDateTap(DateTapDetails details) {
    ...
    if (details.hasEvent) {
      ...
      if (tappedDiary != null) {
        // 표시할 스니펫 텍스트 준비
        String snippet =
            tappedDiary.summary.isNotEmpty
                ? tappedDiary.summary
                : (tappedDiary.content.length > 50
                    ? '${tappedDiary.content.substring(0, 50)}...'
                    : tappedDiary.content);
        if (snippet.isNotEmpty) {
          ...
          if (calendarBox != null && calendarBox.attached) {
            ...
            // 캘린더 위치를 알 수 없을 경우 화면 중앙 부근에 표시
            overlayPosition = Offset(
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '${DateFormat.Md('ko_KR').format(details.date)}의 일기 정보가 캐시에 없습니다.',
            ),
          ),
        );
      }
  ...
  void _removeDiarySnippetOverlay() {
    ...
    if (_diarySnippetOverlayEntry != null) {
      if (_diarySnippetOverlayEntry!.mounted) {
        // 오버레이가 아직 화면에 있는지 확인
  ...
  Widget build(BuildContext context) {
    ...
    switch (_currentIndex) {
      ...
      case 1: // 검색 탭
        currentAppBar =
            _buildHomeAppBar(); // "나만의 AI Assistance, Qnote" 헤더 표시 (UI 변경 요청에 따름)
        break;
      case 2: // AI 채팅 탭
        currentAppBar =
            null; // ChatScreen이 자체 AppBar를 사용하므로 DashboardScreen의 AppBar는 null
        break;
      case 3: // 일정 탭
    ...
    return Scaffold(
      ...
      body: IndexedStack(
        // IndexedStack으로 화면 전환 시 각 탭의 상태 유지
        index: _currentIndex,
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(
            icon: CircleAvatar(
              backgroundColor: Color(0xFFB59A7B),
              child: Icon(
